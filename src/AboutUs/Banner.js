...
import img from "../Images/About us/about.png";
...
function Banner() {
  ...
  useEffect(() => {
    ...
    const textObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          // Har baar screen par aane par transition ho
          setIsTextVisible(true);  // Text visible hone par animation
        } else {
          setIsTextVisible(false); // Jab text viewport se bahar jaaye, reset ho
        }
      });
    }, observerOptions);
    ...
    const imageObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          // Har baar screen par aane par transition ho
          setIsImageVisible(true);  // Image visible hone par animation
        } else {
          setIsImageVisible(false); // Jab image viewport se bahar jaaye, reset ho
        }
      });
    }, observerOptions);
  ...
  }, []);  // Empty dependency array, observer only runs once on mount
  ...
  return (
    <Box data-scroll-section sx={{ px: { sm: 7 }, mt: 10 }}>
      ...
      {/* Call Us Modal */}
      <CallUsButton open={openCallUs} onClose={handleCloseCallUs} />
      {/* Leave Us a Message Modal */}
      <LeaveMessageButton open={openLeaveMessage} onClose={handleCloseLeaveMessage} />
    </Box>
  );
}
